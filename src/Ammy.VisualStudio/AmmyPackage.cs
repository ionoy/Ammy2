...
using Ammy.VisualStudio.Generation;
...
namespace Ammy.VisualStudio
{
    ...
    [Guid(AmmyPackage.PackageGuidString)]
    [ProvideAutoLoad(UIContextGuids80.SolutionExists)]
    [SuppressMessage("StyleCop.CSharp.DocumentationRules", "SA1650:ElementDocumentationMustBeSpelledCorrectly", Justification = "pkgdef, VS and vsixmanifest are valid VS terms")]
    public sealed class AmmyPackage : AsyncPackage
    {
        ...
        private static void AppendTypeConstructors(ITypeSymbol bindableObject, AmmyPageGenerator AmmyPageGenerator)
        {
            foreach (var ctor in bindableObject.GetMembers().OfType<IMethodSymbol>().Where(m => m.MethodKind == MethodKind.Constructor)) {
                ...
                AmmyPageGenerator.AddTypeConstructor(bindableObject.GetFullyQualifedName(), bindableObject.Name, parameterTypes, parameterNames);
            }
        }
        ...
        private void BuildSource(ITypeSymbol type, MetaGenerator metaGenerator)
        {
            if (IsSuitableBindableObject(type)) {
                ...
                AppendTypeConstructors(type, metaGenerator.AmmyPage);
            }
        }
